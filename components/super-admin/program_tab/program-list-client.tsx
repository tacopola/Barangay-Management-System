...
import { toggleProgramStatusAction, deleteProgramAction } from "@/actions/program"
...
export function ProgramListClient({
  ...
  const totalBeneficiaries = programs.reduce((s, p) => s + p.beneficiaryCount, 0)
  ...
  function handleToggle(p: Program) {
    startTransition(async () => {
      ...
      else toast.success(`${p.name} ${p.isActive ? "deactivated" : "activated"}.`)
      router.refresh()
    })
  }
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-4 gap-4">
        <StatCard icon={<Boxes className="h-4 w-4 text-primary" />} label="Total Programs" value={programs.length} accent="border-t-primary" />
        <StatCard icon={<Users className="h-4 w-4 text-emerald-600" />} label="Beneficiaries" value={totalBeneficiaries} accent="border-t-emerald-500" />
        <StatCard icon={<Power className="h-4 w-4 text-muted-foreground" />} label="Active" value={activeCount} accent="border-t-amber-500" />
        <StatCard icon={<Power className="h-4 w-4 text-muted-foreground" />} label="Inactive" value={inactiveCount} accent="border-t-red-500" />
      </div>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b gap-4 flex-wrap">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Tabs value={typeFilter} onValueChange={setTypeFilter}>
              <TabsList className="h-8">
                <TabsTrigger value="all" className="text-xs px-3">All Types</TabsTrigger>
                <TabsTrigger value="4ps" className="text-xs px-3">4Ps</TabsTrigger>
                <TabsTrigger value="senior_citizen" className="text-xs px-3">Senior</TabsTrigger>
                <TabsTrigger value="pwd" className="text-xs px-3">PWD</TabsTrigger>
                <TabsTrigger value="solo_parent" className="text-xs px-3">Solo Parent</TabsTrigger>
                <TabsTrigger value="indigent" className="text-xs px-3">Indigent</TabsTrigger>
              </TabsList>
            </Tabs>
          </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b bg-muted/30">
                <th className="text-left px-5 py-2.5 text-[11px] uppercase tracking-wider text-muted-foreground font-medium">Program</th>
                <th className="text-left px-5 py-2.5 text-[11px] uppercase tracking-wider text-muted-foreground font-medium">Type</th>
                ...
                <th className="text-left px-5 py-2.5 text-[11px] uppercase tracking-wider text-muted-foreground font-medium hidden sm:table-cell">Beneficiaries</th>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((p) => (
                  <tr key={p.id} className="border-b last:border-0 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-5 py-3.5">
                      <span className={`text-[10px] font-semibold px-2.5 py-1 rounded-full border ${PROGRAM_TYPE_COLORS[p.type] ?? ""}`}>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent className="p-8">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This permanently deletes the program. Beneficiary records linked to it will also be removed.
            </AlertDialogDescription>
          </AlertDialogHeader>
