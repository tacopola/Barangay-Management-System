...
import { getBlotterCase } from "@/db/queries/admin/blotter"
import { notFound } from "next/navigation"
import { BlotterCaseClient } from "@/components/admin/blotter_tab/blotter-case-client"
      ...
      <div className="rounded-xl border bg-card p-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <h1 className="text-xl font-bold font-mono">{blotterCase.caseNumber}</h1>
            <p className="text-sm text-muted-foreground mt-1">
              Filed {new Date(blotterCase.createdAt).toLocaleDateString("en-PH", {
                month: "long", day: "numeric", year: "numeric",
              })}
            </p>
          </div>
          <span className={`text-xs font-semibold px-3 py-1.5 rounded-full border ${STATUS_STYLES[blotterCase.status] ?? ""}`}>
        ...
        <div className="grid grid-cols-2 gap-6 mt-6">
          <div>
            <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold mb-1">Complainant</p>
            <p className="text-sm font-medium">{blotterCase.complainantName ?? "—"}</p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold mb-1">Respondent</p>
            <p className="text-sm font-medium">{blotterCase.respondentName ?? "—"}</p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold mb-1">Incident Date</p>
            <p className="text-sm">
        ...
        {/* Narrative */}
        <div className="mt-6">
          <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold mb-2">Narrative</p>
        ...
        {blotterCase.resolution && (
          <div className="mt-4">
            <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold mb-2">Resolution</p>
        ...
        {blotterCase.isEscalated && blotterCase.escalationReason && (
          <div className="mt-4">
            <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold mb-2">Escalation Reason</p>
