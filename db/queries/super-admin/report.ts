...
export const getReportsData = unstable_cache(
  async () => {
    ...
    const barangayStats = await Promise.all(
      allBarangays.map(async (b) => {
        ...
        const [pendingDocs] = await db
          ...
          .where(
            and(
              eq(documentRequests.barangayId, b.id),
        ...
        const [activeBlotter] = await db
          ...
          .where(
            and(
              eq(blotterCases.barangayId, b.id),
    ...
    const monthlyDocs = await db
      .select({
        month: sql<string>`
          to_char(${documentRequests.createdAt}, 'Mon YYYY')
        `,
        count: count(),
      })
      .from(documentRequests)
    ...
    const totalBeneficiaries = barangayStats.reduce(
      (s, b) => s + b.beneficiaries,
      0,
    );
